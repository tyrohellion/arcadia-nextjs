...
import EventCardSearch from "../boxes/EventCardSearch";
...
const PlayerResults = ({ id }) => {
  ...
  const buildUrl = (page) => {
    ...
    if (searchInput) params.set("sort", searchInput);
  ...
  return (
    <>
      ...
      <div className="bar-results-wrapper">
        {results.map((match) =>
          match._id ? (
            <MatchResultsBar
              key={match._id}
              ...
              blueImage={
                match.blue.team.team.image || "/static/images/rocketleague.svg"
              }
              orangeImage={
        ...
        {loading &&
          Array.from({ length: 50 }).map((_, index) => (
            <div className="skeleton-bar" key={index}></div>
          ))}
      </div>
